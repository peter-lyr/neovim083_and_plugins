...
projFileNames = [fname.replace("/", "\\") for fname in projFileNames + [projFileName]]
...
os.system('cd ' + rootdir + ' && del /s /q .cache & rd /s /q .cache & del /s /q build & rd /s /q build & cmake -B build -G "MinGW Makefiles" -DCMAKE_EXPORT_COMPILE_COMMANDS=1 && cd build & copy compile_commands.json ..\ /y & cd ..')
